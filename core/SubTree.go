...
func (this *SubTree) OnTick(tick *Tick) b3.Status {
	...
	sTree := subTreeLoadFunc(this.GetName(), tick.target)
	if nil == sTree {
		return b3.ERROR
	}
...
var subTreeLoadFunc func(string, interface{}) *BehaviorTree

//获取子树的方法
func SetSubTreeLoadFunc(f func(string, interface{}) *BehaviorTree) {
	subTreeLoadFunc = f
}